...
- **密码强度检测**：输入新密码时实时显示强度条（非常弱/弱/中等/强/非常强），基于长度、大小写、数字、特殊字符综合评分
- **后端复杂度校验**：新密码需包含大写字母、小写字母、数字、特殊字符中的至少 3 种
- **安全审计**：修改成功后自动记录安全审计日志
...
**2FA 登录流程**：
当用户启用了 2FA 后，登录流程变为两步：
1. 输入用户名和密码，后端验证通过后返回 `{"2fa_required": true}`
2. 前端自动切换到验证码输入界面，用户输入认证器中的 6 位验证码
3. 调用 `/api/auth/2fa/login-verify/` 完成验证并登录
4. 2FA 验证窗口为 5 分钟，超时后需重新登录
...
- `POST /api/auth/2fa/login-verify/` — 2FA 登录验证
...
- **默认语言**：切换系统界面语言（中文/English），同时同步到后端 UserProfile 模型持久化存储，跨设备登录时自动恢复
- **主题模式**：亮色/暗色/跟随系统
- **主题颜色**：8 种预设主色可选
...
图表使用 ECharts + vue-echarts 渲染，支持暗色主题自适应（文字颜色、背景色、分割线、Tooltip 均随主题切换）。

**后端 API**：`GET /api/user-stats-chart/?days=N`

**缓存策略**：后端使用 Redis 缓存图表数据 5 分钟（key: `chart:stats:{days}`），缓存不可用时自动降级为直接查询数据库。
